\section{The Kiwitsche 2-Punkt-Verfahren}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        >=Stealth,
        phase/.style={draw, rounded corners=4pt, fill=blue!8,
                      minimum width=3.2cm, minimum height=0.9cm,
                      align=center, font=\small},
        arrow/.style={->, thick},
        label/.style={font=\scriptsize, text=gray, align=left},
        node distance=0.45cm,
    ]
        \node[phase] (p1) {\textbf{Phase 1}\\Bang-Bang excitation};
        \node[phase, below=of p1] (p2) {\textbf{Phase 2}\\Extract $\Tu$, $\Au$};
        \node[phase, below=of p2] (p3) {\textbf{Phase 3}\\Identify $\hat K$, $\hat T$, $\hat L$};
        \node[phase, below=of p3] (p4) {\textbf{Phase 4}\\IMC-PID synthesis};
        \node[phase, below=of p4, fill=green!10] (sw) {Switch to PID control};
        ...
        \node[label, right=0.5cm of p1] {Enable BB, wait\\for stationary cycle};
        \node[label, right=0.5cm of p2] {Average over\\$N_{\min}=5$ cycles};
        \node[label, right=0.5cm of p3] {Slope-based $\hat{L}$, $\hat{T}$, $\hat{K}$};
        \node[label, right=0.5cm of p4] {$\lambda = \hat L$};
    \end{tikzpicture}
    \caption{Flow chart of the Kiwitsche 2-Punkt-Verfahren (KZV).}
...
Once a stationary limit cycle is established, let $N \geq N_{\min}$ be the
number of recorded cycles, with $N_{\min} = 5$.
Record $N$ consecutive oscillation cycles and compute:

\begin{align}
    \Tu &= \frac{1}{N_{\min}} \sum_{k=1}^{N_{\min}} (t_{k+1} - t_k), \label{eq:Tu_est} \\
    \Au &= \frac{1}{N_{\min}} \sum_{k=1}^{N_{\min}} \bigl(y_{\max,k} - y_{\min,k}\bigr), \label{eq:Au_est}
\end{align}
...
The three estimates must be computed in the strict order
$\hat{L} \to \hat{T} \to \hat{K}$: $\hat{T}$ depends on $\hat{L}$, and
$\hat{K}$ depends on $\hat{T}$.
...
\begin{remark}
    \eqref{eq:T_est} is exact for $U_{\min} = 0$.
