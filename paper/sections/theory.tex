...
For a stable plant $G(s)$ with sufficient gain and phase delay, the closed
loop with the Bang-Bang controller converges to a periodic
\emph{limit-cycle oscillation}~\cite{Atherton1975}.
...
For the special case of zero hysteresis ($d = 0$), \cref{eq:df} reduces to
the ideal relay describing function $N(A) = 4M/(\pi A)$, and
\cref{eq:hb} becomes equivalent to the Ziegler--Nichols ultimate gain
...
Evaluating \cref{eq:fopdt} at $s = j\omega_u$ gives
\begin{align}
...
Combining \cref{eq:phase_cond,eq:phase_from_df} yields
\begin{equation}
...
With $\omega_u$ known from the measured period $\Tu$, \cref{eq:gain_cond,eq:gain_from_df,eq:phase_id}
...
time), which provides a good trade-off between speed and robustness for
